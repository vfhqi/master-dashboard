...
def compute_all_filters(prices):
    ...
    for stock in prices:
        ...
        # Signal 3: Volume quality (placeholder — needs multi-day vol analysis)
        utr_s3 = "amber"
        utr_s3_score = 0.5

        # Signal 4: Up/down volume ratio (placeholder)
        utr_s4 = "amber"
        utr_s4_score = 0.5

        # Signal 5: Candle quality (placeholder)
        utr_s5 = "amber"
        utr_s5_score = 0.5

        # Signal 6: Distribution days count (placeholder)
        utr_s6 = "amber"
        utr_s6_score = 0.5

        # Signal 7: Pullback contraction (placeholder)
        utr_s7 = "amber"
        utr_s7_score = 0.5
...
def main():
    ...
    if args.full_universe:
        ...
        # Also save as the new universe.json for consistency
    ...
    # stock_mapping_final.json has canonical industry/sector with letter prefixes
    # (e.g. "G. Financials", "H.10. Hotels"). The watchlist has raw names without
    # prefixes. Map every stock through the canonical source.
    ...
    with open(DATA_DIR / "filter-results.json", "w") as f:
        json.dump({
            "_meta": {
                ...
                "notes": "VCP pattern detection pending Phase 2. UTR signals 3-7 are placeholders pending multi-day analysis."
            },
